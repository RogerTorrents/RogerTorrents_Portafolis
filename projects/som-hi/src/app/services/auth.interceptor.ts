...
/**
 * Afegeix `Authorization: Bearer <accessToken>` a les crides cap a
 * `geoexplorer-api` (auth) **i** `som-hi-api` (dades) — totes dues APIs
 * accepten el mateix token perquè comparteixen `JWT_ACCESS_SECRET`. Si una
 * crida privada torna 401 (access token caducat), intenta refrescar-lo un
 * cop via `SessioService.refrescarToken()` (sempre contra `authApiUrl`,
 * l'únic emissor) i reintenta la petició original amb el token nou.
 */
export const authInterceptor: HttpInterceptorFn = (req, next) => {
  const esApiPropia = req.url.startsWith(environment.authApiUrl) || req.url.startsWith(environment.apiUrl);
